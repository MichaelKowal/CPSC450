...
class ProxyMiddleware(object):
    ...
    In this case everything below ``'/assets/'`` is proxied to the server on
    port 5001.  The host header is automatically rewritten and so are request
    URLs (eg: the leading `/assets/` prefix here gets chopped off).
...
class SharedDataMiddleware(object):
    ...
    """A WSGI middleware that provides assets content for development
    ...
    only file names for assets files.
